...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { userId, overrides, analysisType: requestedType } = body;
    const propertyId = body.propertyId && typeof body.propertyId === "string" && body.propertyId.trim() ? body.propertyId.trim() : undefined;
    const projectId = (body.projectId && typeof body.projectId === "string" && body.projectId.trim()) ? body.projectId.trim() : "workspace-default";
    if (!userId) {
    ...
    if (analysisType !== "retail") {
      ...
      if (propertyId) {
        ...
        } catch (e) { console.warn("[score] Property update failed (non-retail):", e); }
      }
    ...
    if (propertyId) {
      ...
      } catch (e) { console.warn("[score] Property update failed (retail):", e); }
    }
    ...
    console.error("Scoring error:", error?.message || error, "| propertyId:", body?.propertyId, "| projectId:", body?.projectId, "| stack:", error?.stack?.slice(0, 300));
