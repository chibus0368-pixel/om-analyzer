...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[parser] Stage 1: Extracting facts (${analysisType})...`);
...
Return JSON only.\n\n${documentText.substring(0, 40000)}`;
    ...
    console.log("[parser] Stage 1 complete:", JSON.stringify(
      isLand
        ? { name: stage1.property?.name, acres: stage1.property?.total_acres, zoning: stage1.zoning?.current_zoning, price: stage1.pricing?.asking_price }
        : { name: stage1.property?.name, tenants: stage1.tenants?.length || 0, hasIncome: !!stage1.income, hasExpenses: !!stage1.expenses }
    ));

    // ===== CROSS-CHECK: Validate extracted data quality (ALL asset types) =====
    const xcheckWarnings: string[] = [];
    const brokerPatterns = /\b(realty|brokerage|advisors|capital|partners|group|associates|investments|commercial|cushman|cbre|jll|marcus\s*&\s*millichap|colliers|newmark|berkadia|kw\s+commercial|lee\s*&\s*associates|svn|nai|coldwell|century\s*21|keller\s+williams|re\/max|sotheby|loopnet)\b/i;
    {
      ...
      if (prop.name && prop.broker) {
        ...
        if (brokerFirstWord.length > 3 && nameLower.includes(brokerFirstWord)) {
          xcheckWarnings.push(`CROSS-CHECK: Property name "${prop.name}" may contain broker info ("${prop.broker}"). Will use address if available.`);
          if (prop.address) {
            console.log(`[parser] Cross-check fix: Replacing name "${prop.name}" with address "${prop.address}"`);
      ...
      if (prop.name && brokerPatterns.test(prop.name)) {
        xcheckWarnings.push(`CROSS-CHECK: Property name "${prop.name}" matches brokerage pattern. Will use address if available.`);
        if (prop.address) {
          stage1.property.name = prop.address;
        }
      }
      ...
      if (prop.name && (prop.name.length < 4 || /^(the\s+)?(property|building|site|subject|offering|land|parcel|tract)$/i.test(prop.name))) {
        xcheckWarnings.push(`CROSS-CHECK: Property name "${prop.name}" is generic. Will prefer address.`);
        if (prop.address) stage1.property.name = prop.address;
      }

      // Check 4: address extracted but no name
      if (!prop.name && prop.address) {
        xcheckWarnings.push(`CROSS-CHECK: No property name found. Using address "${prop.address}" as name.`);
        stage1.property.name = prop.address;
      }

      // Check 5: missing lot acreage for industrial/office
      if ((analysisType === "industrial" || analysisType === "office") && !prop.land_acres) {
        xcheckWarnings.push(`CROSS-CHECK: No lot_acres extracted for ${analysisType} property. Will attempt recovery in Stage 3 addon.`);
      }

      // Check 6: land-specific — missing acreage
      if (isLand && !prop.total_acres && !prop.usable_acres) {
        xcheckWarnings.push(`CROSS-CHECK: No acreage found for land property. Critical data missing.`);
      }

      // Check 7: missing asking price (all types)
      const price = isLand ? stage1.pricing?.asking_price : stage1.pricing?.asking_price;
      if (!price) {
        xcheckWarnings.push(`CROSS-CHECK: No asking_price extracted. Pricing analysis will be limited.`);
      }

      // Check 8: address sanity — should contain a number or known road word
      if (prop.address && !/\d/.test(prop.address) && !/\b(road|street|avenue|blvd|drive|lane|way|highway|hwy|pkwy|ct|circle)\b/i.test(prop.address)) {
        xcheckWarnings.push(`CROSS-CHECK: Address "${prop.address}" may not be a real street address.`);
      }

      // Check 9: name and address are identical (redundant but not an error)
      if (prop.name && prop.address && prop.name.toLowerCase().trim() === prop.address.toLowerCase().trim()) {
        xcheckWarnings.push(`CROSS-CHECK: Property name and address are identical ("${prop.name}"). Normal for address-based naming.`);
      }
    }
    if (xcheckWarnings.length > 0) {
      console.warn(`[parser] Cross-check (${analysisType}): ${xcheckWarnings.length} warnings:`, xcheckWarnings);
      await runRef.update({ warningCount: xcheckWarnings.length, crossCheckWarnings: xcheckWarnings });
    }
    ...
    console.log(`[parser] Stage 2: ${isLand ? "Analyzing land deal" : "Calculating underwriting"}...`);
    ...
    const stage2Response = await callOpenAI([
      ...
      { role: "user", content: `${isLand ? "Analyze this land deal" : "Calculate complete underwriting"} from these extracted facts:\n\n${stage2Input}` },
    ], 8000);
    ...
    console.log("[parser] Stage 2 complete:", JSON.stringify({
      ...
      ...(isLand ? { hasSiteAssessment: !!stage2.site_assessment } : { hasDebt: !!stage2.debt }),
    }));
    ...
    if (analysisType && analysisType !== "retail") {
      ...
      if (addonPrompt) {
        try {
          console.log(`[parser] Stage 3: Extracting ${analysisType} addon fields...`);
          const apiKey = process.env.OPENAI_API_KEY;
          const addonResponse = await fetch("https://api.openai.com/v1/chat/completions", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
          ...
          if (addonResponse.ok) {
            ...
            const addonJson = addonRaw.replace(/```json?\s*/g, "").replace(/```/g, "").trim();
            ...
            if (addonFieldsToSave.length > 0) {
              ...
              console.log(`[parser] Saved ${addonFieldsToSave.length} ${analysisType} addon fields`);
            }

            // If addon found lot_acres that Stage 1 missed, backfill it
            if (addonFields.lot_acres && !stage1.property?.land_acres) {
              console.log(`[parser] Backfilling lot_acres from ${analysisType} addon: ${addonFields.lot_acres}`);
              ...
              const backfillRef = db.collection("workspace_extracted_fields").doc();
            ...
            console.warn(`[parser] ${analysisType} addon extraction API returned ${addonResponse.status}`);
          }
        } catch (addonErr) {
          console.warn(`[parser] ${analysisType} addon extraction failed:`, addonErr);
    ...
    if (isLand) {
      ...
      saveField("property_basics", "asset_type", p.asset_type || "land", 0.95, "confirmed");
      saveField("property_basics", "lot_acres", p.total_acres, 0.9, "confirmed");
      saveField("property_basics", "usable_acres", p.usable_acres, 0.85, "confirmed");
      saveField("property_basics", "lot_dimensions", p.lot_dimensions, 0.8, "confirmed");
      saveField("property_basics", "frontage_ft", p.frontage_ft, 0.8, "confirmed");
      ...
      saveField("pricing_deal_terms", "price_per_acre", pr.price_per_acre, 0.9, "calculated");
      saveField("pricing_deal_terms", "price_per_sf", pr.price_per_sf, 0.85, "calculated");
      saveField("pricing_deal_terms", "pricing_signal", pr.pricing_signal, 0.8, "calculated");

      // Zoning
      const z = stage1.zoning || {};
      saveField("land_zoning", "current_zoning", z.current_zoning, 0.9, "confirmed");
      saveField("land_zoning", "zoning_description", z.zoning_description, 0.8, "confirmed");
      saveField("land_zoning", "planned_use", z.planned_use, 0.85, "confirmed");
      saveField("land_zoning", "entitled", z.entitled, 0.8, "confirmed");
      saveField("land_zoning", "entitlement_status", z.entitlement_status, 0.8, "confirmed");
      saveField("land_zoning", "permitted_uses", z.permitted_uses, 0.8, "confirmed");
      saveField("land_zoning", "density_allowed", z.density_allowed, 0.8, "confirmed");
      ...
      saveField("land_access", "highway_proximity", a.highway_proximity, 0.8, "confirmed");
      saveField("land_access", "frontage_description", a.frontage_description, 0.8, "confirmed");
      ...
      saveField("land_environmental", "phase1_complete", env.phase1_complete, 0.8, "confirmed");
      saveField("land_environmental", "environmental_notes", env.environmental_notes, 0.8, "confirmed");
      saveField("land_environmental", "soil_conditions", env.soil_conditions, 0.7, "confirmed");
      saveField("land_environmental", "wetlands", env.wetlands, 0.7, "confirmed");

      // Site assessment (from Stage 2)
      const sa = stage2.site_assessment || {};
      saveField("land_assessment", "development_readiness", sa.development_readiness, 0.8, "calculated");
      saveField("land_assessment", "highest_best_use", sa.highest_and_best_use, 0.8, "calculated");
      saveField("land_assessment", "entitlement_risk", sa.entitlement_risk, 0.8, "calculated");
      ...
      if (parsed.pricing) {
        ...
        saveField("pricing_deal_terms", "lease_rate_monthly", pr.lease_rate_monthly, 0.85, "confirmed");
    ...
    if (isLand && stage2.validation && Array.isArray(stage2.validation)) {
      for (let i = 0; i < stage2.validation.length; i++) {
        saveField("validation", `check_${i + 1}`, stage2.validation[i], 0.9, "calculated");
      }
    }
    ...
    if (brief && propertyId) {
      try {
        await db.collection("workspace_notes").add({
          ...
          title: isLand ? "First-Pass Land Analysis Brief" : "First-Pass Investment Brief",
          content: brief,
          isPinned: true,
          createdAt: now,
          updatedAt: now,
    ...
    if (propertyId) {
      try {
        ...
        const nameIsBrokerLike = propName && (
          brokerPatterns.test(propName) ||
          (propBroker && propName.toLowerCase().includes(propBroker.toLowerCase().split(/\s+/)[0])) ||
          (propBroker && propBroker.toLowerCase().includes(propName.toLowerCase().split(/\s+/)[0]) && propName.length < 30)
        );

        // If name looks like broker info, replace with address
        if (nameIsBrokerLike && propAddress) {
          console.log(`[parser] Cross-check: Property name "${propName}" looks like broker info. Using address: "${propAddress}"`);
        ...
        if (!propName || propName === "Unknown Property") {
          if (propAddress && propAddress !== "Unknown Address") {
            ...
            const firstLine = documentText.split("\n").find((line: string) => {
              ...
              return trimmed.length > 5 && trimmed.length < 80
                && !trimmed.startsWith("---") && !trimmed.startsWith("===")
                && !brokerPatterns.test(trimmed)
                && !/^(confidential|offering|memorandum|prepared|presented|exclusively)/i.test(trimmed);
            });
        ...
        if (isLand) {
          ...
          if (propData.usable_acres) propUpdate.usableAcres = propData.usable_acres;
        } else {
          if (propData.gla_sf) propUpdate.buildingSf = propData.gla_sf;
          if (propData.occupancy_pct) propUpdate.occupancyPct = propData.occupancy_pct;
          ...
          if (propData.vacancy_status) propUpdate.vacancyStatus = propData.vacancy_status;
          if (propData.building_count) propUpdate.buildingCount = propData.building_count;
    ...
    return NextResponse.json({
      ...
      brief: brief,
      fields: isLand
        ? { property: stage1.property, pricing: { ...(stage1.pricing || {}), ...(stage2.pricing || {}) }, zoning: stage1.zoning, utilities: stage1.utilities, access: stage1.access, environmental: stage1.environmental, site_assessment: stage2.site_assessment, signals: stage2.signals, validation: stage2.validation }
        : { property: stage1.property, pricing: { ...(stage1.pricing || {}), ...(stage2.pricing || {}) }, income: { ...(stage1.income || {}), ...(stage2.income || {}) }, expenses: { ...(stage1.expenses || {}), ...(stage2.expenses || {}) }, debt: stage2.debt, breakeven: stage2.breakeven, exit: stage2.exit, tenants: stage1.tenants, signals: stage2.signals, validation: stage2.validation },
